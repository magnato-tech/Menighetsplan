...
export const KalenderView: React.FC<KalenderViewProps> = ({
  ...
  const [visSynkListe, setVisSynkListe] = useState(true);
  const kalenderArkRef = useRef<HTMLDivElement>(null);
  ...
  const visSynkOppgaver = visSynkListe && apneOppgaver.length > 0;
  ...
  const kjorSynk = async () => {
    ...
    setSynkStatus("");
    try {
      const ics = await hentEksternIcalTekst(
        getCustomScriptUrl(),
        undefined,
        DEFAULT_REMOTE_SCRIPT_URL,
        hentInnstillinger(dbRef.current).eksternIcalUrl
      );
      ...
      setSynkStatus(
        resultat.nye
          ? `${resultat.nye} hendelse(r) fra kalenderfeeden mangler i appen.`
          : "App-kalenderen dekker feeden. Ingen nye oppgaver."
      );
    } catch (err) {
  ...
  const hoppTilKalender = (dato?: string) => {
    ...
    requestAnimationFrame(() => {
      kalenderArkRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
      {visSynkOppgaver && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3 lg:sticky lg:top-2 lg:max-h-[min(70vh,40rem)] lg:overflow-y-auto">
          ...
          <p className="text-xs text-amber-900">
            ...
            <button
              type="button"
              onClick={() => hoppTilKalender()}
              className="min-h-11 px-3 py-1.5 bg-white border border-amber-200 text-amber-950 text-xs font-semibold rounded-xl inline-flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => setVisSynkListe(false)}
              className="p-2 min-h-11 min-w-11 rounded-xl text-amber-900 hover:bg-amber-100 cursor-pointer"
              aria-label="Lukk synklisten"
            >
              <X className="w-5 h-5" />
            </button>
            </div>
          ...
          <ul className="space-y-2">
            {apneOppgaver.map((o) => {
              ...
              const allerede = hendelser.filter((h) => h.dato === o.Dato);
              return (
              <li
                ...
                <div className="flex-1 text-sm">
                  ...
                  {allerede.length > 0 ? (
                    <p className="text-xs text-emerald-800 mt-1">
                      Allerede i appen: {allerede.map((h) => `${h.tid} ${h.tittel}`).join(" · ")}
                    </p>
                  ) : (
                    <p className="text-xs text-slate-500 mt-1">Ingen hendelse i appen denne dagen.</p>
                  )}
                  <button
                    type="button"
                    onClick={() => hoppTilKalender(o.Dato)}
                    className="mt-1 text-xs font-semibold text-[#2d5a3f] underline cursor-pointer"
                  >
                    Vis denne dagen
                  </button>
                  {maler.length > 0 && (
